...
# Work in progress
<p>Right now I'm testing on a breadboard while waiting for some parts to arrive and the only things implemented are the LED multiplexing and the once per second intrerrupt from the timer ~~using the main oscillator as I still need to get some appropriate crystals for the dedicated oscillator~~. Added the external cystal for the dedicated oscillator and working on the UART time sync.</p>
